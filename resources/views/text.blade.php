<!DOCTYPE html>
<html>
<head>
	<meta charset="utf-8">
	<title>Test</title>
	<meta name="viewport" content="width=device-width, initial-scale=1">

	<!-- Facebook and Twitter integration -->
	<meta property="og:title" content=""/>
	<meta property="og:image" content=""/>
	<meta property="og:url" content=""/>
	<meta property="og:site_name" content=""/>
	<meta property="og:description" content=""/>
	<meta name="twitter:title" content="" />
	<meta name="twitter:image" content="" />
	<meta name="twitter:url" content="" />
	<meta name="twitter:card" content="" />


	<link rel="stylesheet" href="../bootstrap-4.1.3-dist/css/bootstrap.min.css">
	<link rel="stylesheet" type="" href="../fontawesome-free-5.3.1-web/css/all.css">
	<!-- Animate.css -->
	<link rel="stylesheet" href="../public/css/animate.css">
	<!-- Icomoon Icon Fonts-->
	<link rel="stylesheet" href="../public/css/icomoon.css">
	<!-- Bootstrap  -->
	<link rel="stylesheet" href="../public/css/bootstrap.css">

	<!-- Magnific Popup -->
	<link rel="stylesheet" href="../public/css/magnific-popup.css">

	<!-- Owl Carousel  -->
	<link rel="stylesheet" href="../public/css/owl.carousel.min.css">
	<link rel="stylesheet" href="../public/css/owl.theme.default.min.css">
	<!-- Flexslider  -->
	<link rel="stylesheet" href="../public/css/flexslider.css">

	<!-- Theme style  -->
	<link rel="stylesheet" href="../public/css/style.css">
	<!-- Modernizr JS -->
	<script src="../public/js/modernizr-2.6.2.min.js"></script>
    <script src="../public/js/wow.min.js"></script>
	<!--[if lt IE 9]>
	<script src="js/respond.min.js"></script>
	<![endif]-->

</head>
<body>
	<div class="fh5co-loader"></div>
	
	<div id="page">
	<nav class="fh5co-nav" role="navigation">
		<div class="top-menu">
			<div class="container-fluid">
				<div class="row">
					<div class="col-md-4 col-sm-11 nav-logo">
						<div id="fh5co-logo"><a href="index.html"><img src="../resources/img/HyperDevVectFin.png" width="70" height="70" class="d-inline-b" style="margin:0; padding: 0;"><span>HyperDev</span></a></div>
					</div>
					<div class="col-md-8 col-sm-1 text-right menu-1">
						<ul>
							<li class="active"><a href="#">HOME</a></li>
							<li><a href="#services">SERVICES</a></li>
							<li><a href="#">PORTFOLIO</a></li>
							<li><a href="#">CONTACT</a></li>
							<li class="has-dropdown">
								<a href="#">LANGAGE</a>
								<ul class="dropdown">
									<li><a class="none" href="#">English</a></li>
									<li><a class="none" href="#">French</a></li>
								</ul>
							</li>
						</ul>
					</div>
				</div>
				
			</div>
		</div>
	</nav>

	<aside id="fh5co-hero" class="js-fullheight">
		<div class="flexslider js-fullheight">
			<ul class="slides">
		   	<li style="background-image: url(../resources/img/bg.jpg);">
		   		<div class="overlay-gradient"></div>
		   		<div class="container">
		   			<div class="row">
			   			<div class="col-md-8 col-md-offset-2 text-center js-fullheight slider-text">
			   				<div class="slider-text-inner">
			   					<h1>HyperDev</h1>
									<h2>Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua.</h2>
									<p><a class="btn btn-primary btn-lg" href="#about">Read More</a></p>
			   				</div>
			   			</div>
			   		</div>
		   		</div>
		   	</li>
		   	<li style="background-image: url(../resources/img/bg1.jpg);">
		   		<div class="overlay-gradient"></div>
		   		<div class="container">
		   			<div class="row">
			   			<div class="col-md-8 col-md-offset-2 text-center js-fullheight slider-text">
			   				<div class="slider-text-inner">
			   					<h1>Join us</h1>
									<h2>Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua.</h2>
									<p><a class="btn btn-primary btn-lg btn-learn" href="#">Get Started</a></p>
			   				</div>
			   			</div>
			   		</div>
		   		</div>
		   	</li>	   	
		  	</ul>
	  	</div>
	</aside>
    <section class="about-container wow fadeInDown" id="about">
        <div class="container-fluid">
            <div class="row">
                <div class="col-md-3">
                    <img src="../resources/img/HyperDevVectFin.png" class="img-fluid rounded-circle about-logo">
                </div>
                <div class="col-md-9">
                    <h1>About <span>HyperDev</span></h1>
                    <p>Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua.Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua.Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua.Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua.Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua.Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua.Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua.</p>
                </div>
            </div>
        </div>
    
    </section>
        
    <section class="service-container" id="services">
        <div class="container-fluid">
            <div class="row">
                <div class="col-sm-12">
                    <h1 class="wow fadeInDown">Our Services</h1>
                </div>
            </div>
        <div class="container-fluid">
            <div class="row">
                <div class="col-lg-12">
                    <div class="service-content wow fadeIn">
                        <div class="content-icons">
                            <div class="service-content-icons">
                                <div class="service_rotated rotating">
                                </div>
                                <div class="service_center">
                                    <img src="../resources/img/logo_responsive.png" class="img-fluid">
                                </div>
                                <div class="service_icons icons-1">
                                    <i class="fas fa-desktop"></i>
                                </div>
                                <div class="content-items content-1">
                                    <h1>Web design</h1>
                                    <p>Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua.</p>
                                </div>
                                <div class="service_icons icons-2">
                                    <i class="fas fa-cogs"></i>
                                </div>
                                <div class="content-items content-2">
                                    <h1>Mobile Apps</h1>
                                    <p>Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua.</p>
                                </div>
                                <div class="service_icons icons-3">
                                    <i class="fas fa-globe-americas"></i>
                                </div>
                                <div class="content-items content-3">
                                    <h1>Maintenance</h1>
                                    <p>Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua.</p>
                                </div>
                                <div class="service_icons icons-4">
                                    <i class="fas fa-mobile-alt"></i>
                                </div>
                                <div class="content-items content-4">
                                    <h1>Web Service</h1>
                                    <p>Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua.</p>
                                </div>
                            </div>
                        </div>
                        <div class="service_items items-1">
                            <h1>Web design</h1>
                            <p>Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua.</p>
                        </div>
                        <div class="service_items items-2">
                            <h1>Mobile Apps</h1>
                            <p>Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua.</p>
                        </div>
                        <div class="service_items items-3">
                            <h1>Maintenance</h1>
                            <p>Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua.</p>
                        </div>
                        <div class="service_items items-4">
                            <h1>Web service</h1>
                            <p>Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua.</p>
                        </div>
                    </div>
                </div>
             </div>
                
            </div>
        </div>
    </section>
    
        
	<footer id="fh5co-footer" class=" wow fadeInDown" role="contentinfo">
		<div class="container-fluid">
			<div class="row ">
				<div class="col-md-3 fh5co-widget logo-footer">
					<img src="../resources/img/HyperDevVectFin.png" width="200" height="200">
				</div>
				<div class="col-sm-4 col-md-3 fh5co-widget">
					<h4>HyperDev</h4>
					<p>Facilis ipsum reprehenderit nemo molestias. Aut cum mollitia reprehenderit. Eos cumque dicta adipisci architecto culpa amet.</p>
				</div>
				<div class="col-sm-4 col-md-3 col-md-push-1 footer-nav">
					<h4>Navigation</h4>
					<ul class="fh5co-footer-links">
						<li><a href="#">Home</a></li>
						<li><a href="#">About</a></li>
						<li><a href="#">Portfolio</a></li>
						<li><a href="#">Contacts</a></li>
					</ul>
				</div>

				<div class="col-sm-4 col-md-3 col-md-push-1">
					<h4>Contact Information</h4>
					<ul class="fh5co-footer-links">
						<li>Antananarivo, <br> Andravohangy</li>
						<li>+261 34 21 953 80</li>
						<li><a href="#">HyperDev@gmail.com</a></li>
					</ul>
				</div>

			</div>

			<div class="row copyright">
				<div class="col-md-12 text-center">
					<p>
						<small class="block">&copy; 2018 HyperDev. All Rights Reserved.</small> 
						<small class="block">Designed by <a href="#" target="_blank">HyperDev</a></small>
					</p>
					
						<ul class="fh5co-social-icons">
							<li><a href="#"><i class="icon-twitter"></i></a></li>
							<li><a href="#"><i class="icon-facebook"></i></a></li>
							<li><a href="#"><i class="icon-linkedin"></i></a></li>
						</ul>
					
				</div>
			</div>

		</div>
	</footer>

<!--
	<div class="cookie-bar">
		<h4>Welcome!</h4>
		<p>Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua.</p>
		<a href="#">Got it!</a>
	</div>
-->
	
	</div>

	<div class="gototop js-top">
		<a href="#" class="js-gotop"><i class="icon-arrow-up"></i></a>
	</div>



	<!-- jQuery -->
	<script src="../bootstrap-4.1.3-dist/js/jquery-3.3.1.slim.min.js"></script>
	<script src="../bootstrap-4.1.3-dist/js/popper.min.js"></script>
	<script src="../bootstrap-4.1.3-dist/js/bootstrap.min.js"></script>

	<script src="../public/js/jquery.min.js"></script>
	<!-- jQuery Easing -->
	<script src="../public/js/jquery.easing.1.3.js"></script>
	<!-- Bootstrap -->
	<script src="../public/js/bootstrap.min.js"></script>
	<!-- Waypoints -->
	<script src="../public/js/jquery.waypoints.min.js"></script>
	<!-- Stellar Parallax -->
	<script src="../public/js/jquery.stellar.min.js"></script>
	<!-- Carousel -->
	<script src="../public/js/owl.carousel.min.js"></script>
	<!-- Flexslider -->
	<script src="../public/js/jquery.flexslider-min.js"></script>
	<!-- countTo -->
	<script src="../public/js/jquery.countTo.js"></script>
	<!-- Magnific Popup -->
	<script src="../public/js/jquery.magnific-popup.min.js"></script>
	<script src="../public/js/magnific-popup-options.js"></script>
	<!-- Main -->
	<script src="../public/js/main.js"></script>
    <!-- wow -->
    <script src="../public/js/wow.min.js"></script>
    
</body> 
</html>